...
#             raise ValueError(f'{self.mask_dir} already exists and is not a folder')
        ...
        self.lasso.set_visible(True)#(wi=2048,hi=1532)
    ...
    def onselect(self,verts):
        self.verts = verts
        p = Path(verts)
        #hight, width, lixo = self.img.shape
        self.indices = p.contains_points(self.pix, radius=0).reshape(self.shape[0], self.shape[1])#H e W
